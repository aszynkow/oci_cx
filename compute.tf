resource oci_core_instance appvm {
 ...
 count = local.app_vm_count
  ...
  create_vnic_details {
    ...
    display_name = join(var.env_name,[var.app_vm_hostname[count.index],count.index])#"LPORSHSSAPP162"
  
    hostname_label = join(var.env_name,[var.app_vm_hostname[count.index],count.index])#"lporshssapp162"
  ...
  fault_domain = var.app_fds[count.index]
