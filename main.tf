...
#    )
    ...
    bastion_app_sesions_count = var.do_bastion_sessions ? local.app_vm_count : 0
    #bastion_web_sesions_count = var.do_bastion_sessions ? local.vm_count : 0
    do_mount_count = var.do_mount ? local.app_vm_count : 0
    do_attach_count = var.do_attach ? local.app_vm_count : 0

    lb_public_count = var.do_public_lbs ? 1 : 0 
    ...
    Okit_Vcn001_id = data.oci_core_vcns.vcns.virtual_networks.0.id
    #Okit_Drg001_id = var.existing_drg_id
    Okit_Sg001_id = data.oci_core_service_gateways.service_gateways.service_gateways.0.id
    Okit_Ng001_id = data.oci_core_nat_gateways.nat_gateways.nat_gateways.0.id
    vcn_ip_range = data.oci_core_vcns.vcns.virtual_networks.0.cidr_block
    Okit_Igw01_id = data.oci_core_internet_gateways.internet_gateways.gateways.0.id
    Okit_Vcn001_dhcp_options_id = data.oci_core_vcns.vcns.virtual_networks.0.default_dhcp_options_id
    ...
    subnet_name = join("",[local.env_name,"privsub"])
